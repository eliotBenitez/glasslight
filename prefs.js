...
export default class TahoePreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        const settings = this.getSettings('org.gnome.shell.extensions.tahoe-spotlight');
        const page = new Adw.PreferencesPage({title: 'Spotlight', icon_name: 'system-search-symbolic'});
        ...
        const shortcut = new Adw.EntryRow({title: _('Hotkey (for example, <Alt>space)'), use_markup: false, text: settings.get_strv('toggle-spotlight')[0] ?? ''});
        ...
        shortcut.connect('apply', () => {
            ...
            if (valid && key !== 0) settings.set_strv('toggle-spotlight', [shortcut.text]);
            else shortcut.text = settings.get_strv('toggle-spotlight')[0] ?? '';
        });
